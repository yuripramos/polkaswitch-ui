...
export const Router = ({ web3Provider, signer, chainData }) => {
  const [txManager, setTxManager] = useState();
  ...
  const getDecimals = async (assetId, provider) => {
  ...
  const addLiquidity = async (assetId, liquidityToAdd, infiniteApprove) => {
  ...
  const refreshBalances = async () => {
    ...
    const entries = await Promise.all(
      CHAINS.map(async (chainId) => {
        ...
        return balanceEntries.filter((x) => !!x);
      }),
    );
    const liquidityTable = entries.filter((x) => !!x).flat();

    setBalances(liquidityTable);
  };

  // Returns value in human readable units
  const getLiquidity = async (assetId) => {
    if (!txManager) {
      throw new Error("Needs signer");
    }
