...
export default function TradingViewChart(){
  ...
  console.log('###  initTokenPair ###', initTokenPair);
  ...
  useEffect(() =>{
    const fetchData = async () => {
      const baseUrl = TokenListManager.getCurrentNetworkConfig().tradeView.baseUrl;
      const fromTimeStamp = 1628796813;
      const toTimeStamp = 1629056013
      if (selectedPair.fromSymbol && selectedPair.toSymbol) {
        const fromTokenPrices = await fetchPriceData(baseUrl, selectedPair.fromAddress, 'usd', fromTimeStamp, toTimeStamp);
        console.log('fromTokenPrice', fromTokenPrices)
        const toTokenPrices = await fetchPriceData(baseUrl, selectedPair.toAddress, 'usd', fromTimeStamp, toTimeStamp);
        console.log('toTokenPrice', toTokenPrices)
        const tokenPrices = mergePrice(fromTokenPrices, toTokenPrices)
      } else {
        const tokenPrices = fetchPriceData(baseUrl, selectedPair.fromAddress, 'usd', fromTimeStamp, toTimeStamp);
      }
    };
